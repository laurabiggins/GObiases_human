...
human_transcript_summary %>%
    ...
    geom_label(data = category_counts, colour = dark_b_blue, fill = "lightgrey", aes(label = paste0("n=",n), x = 1:3, y = 410)) #+
...
suspects_for_violin  %>%
   ...
   # geom_boxplot(width = 0.1) 
    ...
    geom_label(data = total_counts, colour = dark_b_blue, fill = "lightgrey", aes(label = paste0("n=",n), x = 1:3, y = 100)) #+
 # 
```
...
dups <- suspects_for_violin %>%
  ...
  count(n)
```
There are `r filter(dups, n ==3) %>% pull(nn)` functional categories that were 
identified in all 3 transcript categories and `r filter(dups, n == 2) %>% pull(nn)`
